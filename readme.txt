...
You don't need to reinstall from scratch. Just replace these 4 files in your existing plugin folder with the versions from this repo, then reload the gateway settings page once to pick up the new option:
...
= 2.1.4 (fork) =
* Fix a confirmation deadlock in viewkey mode: an order requiring more than 0 confirmations could get permanently stuck re-polling forever without ever confirming, if its transaction's block scrolled past the explorer's scan window before its height could be resolved. Confirmed fixed against a live order that had been stuck for over 90 cron cycles.
